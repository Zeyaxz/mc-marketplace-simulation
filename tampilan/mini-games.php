<div class="list">
    <ul>
        ...
        <li class="active"><a>mini games</a></li>
    </ul>
</div>
...
<div class="market-konten">
    <div class="judul">
        <h3>RECENTLY ADDED MINI GAMES</h3>
    </div>
    <div class="isi-konten" id="menu1">
        <?php
        if(isset($_POST["cari"])){
            $tampilkan = $_POST["keyword"];
            $sql = "SELECT * FROM data WHERE judul LIKE '%$tampilkan%' AND `kategori` = 'mini games' ORDER BY `data`.`id` DESC";
        }else{
            $sql = "SELECT * FROM `data` WHERE `kategori` = 'mini games' ORDER BY `data`.`id` DESC";   
        }
        $tampilkan = mysqli_query($connect, $sql);
        while ($data = mysqli_fetch_assoc($tampilkan)): ?>
            <a href=<?= "tampilan.php?id=" . $data['id']?>>
