<div class="list">
    <ul>
        ...
        <li class="active"><a>adventure maps</a></li>
...
<div class="market-konten">
    ...
    <div class="isi-konten" id="menu1">
        ...
        if(isset($_POST["cari"])){
            $tampilkan = $_POST["keyword"];
            $sql = "SELECT * FROM data WHERE judul LIKE '%$tampilkan%' AND `kategori` = 'maps' ORDER BY `data`.`id` DESC";
        }else{
            $sql = "SELECT * FROM `data` WHERE `kategori` = 'maps' ORDER BY `data`.`id` DESC";   
        }
        $tampilkan = mysqli_query($connect, $sql);
        while ($data = mysqli_fetch_assoc($tampilkan)): ?>
            <a href=<?= "tampilan.php?id=" . $data['id']?>>
